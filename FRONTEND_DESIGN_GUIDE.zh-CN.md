...
- 失败：red
- 来源标识应使用 `TagChip` 或轻量 badge，不再在论文卡片顶部使用来源色条。
...
- 弹窗内支持左右切换，上下滚动浏览内容。
- 详情弹窗必须继续使用系统 `ModalWrapper`，宽度用 `minWidth` + `maxWidth` 同时控制；不要只设置 `maxWidth`，因为系统弹窗在大屏下会按内容 `auto` 收缩。当前详情弹窗参考早期视觉宽度使用 `min(68rem, calc(100vw - 8rem))`。
...
所有页面头部、空状态和 sidebar 标题都必须有明确的 locale 文案，不能让用户看到 `Description for ...`、`empty.*.title`、`empty.*.description` 这类 fallback key。
